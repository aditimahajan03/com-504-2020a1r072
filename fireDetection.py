...
fire_cascade = cv2.CascadeClassifier('D:\\Programs\\python\\fire alarm\\Fire_detection_python_project_git\\fire_detection_cascade_model.xml') # To access xml file which includes positive and negative images of fire. (Trained images)
...
def play_alarm_sound_function(): # defined function to play alarm post fire detection using threading
    playsound.playsound('D:\\Programs\\python\\fire alarm\\Fire_detection_python_project_git\\fire_alarm.mp3',True) # to play alarm # mp3 audio file is also provided with the code.
